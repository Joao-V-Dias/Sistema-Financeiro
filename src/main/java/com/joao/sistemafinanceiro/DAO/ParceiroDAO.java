...
public class ParceiroDAO implements GenericDAO<Parceiro>{
	...
	public void salvar(Parceiro p){
		try{
			PreparedStatement stmt = conn.prepareStatement("INSERT INTO parceiro (nome, documento, tipo, email, telefone) VALUES" + "(?, ?, ?, ?, ?)");
	...
	public List<Parceiro> buscarTodos(String tipo){
		List<Parceiro> lstP = new ArrayList<>();
		ResultSet rs;
		try{
	...
	@Override
	public Parceiro consultarResult(ResultSet rs) throws SQLException{
	 ...
	 *
	 * @param documento Usado para identificar a tupla no banco de dados
	 */
