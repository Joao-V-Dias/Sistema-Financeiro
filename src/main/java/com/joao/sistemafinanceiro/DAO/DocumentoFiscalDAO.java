...
public class DocumentoFiscalDAO implements GenericDAO<DocumentoFiscal>{
	Connection conn;
	...
	public void salvar(DocumentoFiscal d){
		try{
			PreparedStatement stmt = conn.prepareStatement("INSERT INTO documento_fiscal (tipo, numero, data_emissao, cliente, fornecedor, valor_total) VALUES (?, ?, ?, ?, ?, ?)");
	...
	@Override
	public List<DocumentoFiscal> buscarTodos(){
	...
	@Override
	public DocumentoFiscal consultarResult(ResultSet rs) throws SQLException{
	...
	@Override
	public void editar(DocumentoFiscal d){
		try{
	 ...
	 *
	 * @param numero Usado para identificar a tupla no banco de dados
	 */
